...
class OrdersReal extends UserEventsInterface {
  ...
  onAccountUpdate(balance, pnl, positions) {
    console.log(
      'ON_ACCOUNT_UPDATE balance=' + balance + ' pnl=' + pnl + ' positions: '
    );
  ...
  cleanSLTPOrders(broker_SL_or_TP_OrderId) {
    ...
    let order = this.orders.find(
      (o) => o.brokerSLID == id || o.brokerTPID == id
    );
    if (!order) {
      return;
    }
    ...
    console.log(
      'OM: found SL/TP orders to clean up: order=',
      order.entry.id,
      ' removeId=',
      removeId
    );
    ...
    this.broker
      .closeOrderIds(order.getSymbol(), [removeId])
      .then((r) => {
