...
class EntryPlan {
  ...
  genFilterFunction(params) {
    ...
    if (p.SYMBOLS) {
      jsf.push("(['" + p.SYMBOLS.join("','") + "'].includes(o.entry.symbol))");
    }
    if (p.TIMEFRAMES) {
      jsf.push(
        "(['" + p.TIMEFRAMES.join("','") + "'].includes(o.entry.timeframe))"
      );
    }
    if (p.STRATEGIES) {
      jsf.push(
        "(['" + p.STRATEGIES.join("','") + "'].includes(o.entry.strategy))"
      );
    }
    if (p.TAGS) {
      for (var t in p.TAGS) {
  ...
  calcGain(order) {
    ...
    const commissionInUSD =
      soldInUSD * this.params.COST_SELL_PERCENT +
  ...
  processEntriesHistory(params, entries) {
    console.log(
      'EP: recalc: processing history entries -> orders ' + TH.ls(TH.now())
    );
