/*
 ** Bitcoin trend, relies on analyzer BTCTREND, transforms flags into entry tags
 */
...
class BTC extends Tagger {
    ...
    BTC.MA_PERIODS.forEach((p) => {
      tags.push({
        ...
        desc:
          'Pass (P) if ' +
          BTC.SYMBOL +
          ' is above (buy entries) or below (sell entries) ' +
          'EMA ' +
          p +
          ' on current timeframe, (F) otherwise. When not enough data returns (N).',
      });
    });
    return tags;
  }
  ...
  createTag(entry, flags, timeframe, period) {
    ...
    const doFilter =
      (btcTrend > 0 && !entry.isLong) || (btcTrend < 0 && entry.isLong);
