...
class EntryFinder {
  ...
  addCandle(candle, io, candleIsCurrent) {
    ...
    if (!this.secondBottom) {
      ...
      if (this.readyToSpotSecondBottom() && this.candleTouchesZone(candle)) {
        if (this.wfcandle) {
          // if there was an extremum (true neck)
  ...
  bottomsNotOkReason(io) {
    ...
    touchFirst.ids.forEach((tfId) => {
      if (touchSecond.ids.includes(tfId)) {
        sameLevel = true;
      }
    });
    if (!sameLevel) {
  ...
  canEnterAfterB2(candleAfterB2) {
    if (!this.candleAboveZone(candleAfterB2)) {
      return false;
    }
    const colorOk =
      (this.isLong && candleAfterB2.isGreen()) ||
...
class DBLBOTTOM extends Strategy {
  ...
  newFinder(isLong, startCandle, io, currentCandle) {
    const exists = this.finders.find(
      (f) => f.isLong == isLong && f.firstBottom === startCandle
    );
    if (exists) {
      return null;
    }
    ...
    for (var c of sinceCandles) {
      if (c !== currentCandle) {
        // currentCandle will be processed in main cycle
