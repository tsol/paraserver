...
class LivePulser extends BrokerEventsCandlesInterface {
  ...
  switchLive() {
    ...
    console.log(
      'CSEQ: cached pulses done, releasing buffered live candles if any:'
    );
  ...
  pulseStart(pulseTime) {
    ...
    this.timeframes.forEach((t) => {
      if ((pulseTime + 1) % TF.get(t).length == 0) {
        expectTimeframes.push(t);
      }
    });
  ...
  pulseRelease() {
    ...
    if (!this.isLive) {
      ...
      const priceUpdates = this.bufferedPriceUpdates.filter(
        (pu) => pu.eventTime <= this.currentPulseTime
      );

      this.bufferedPriceUpdates = this.bufferedPriceUpdates.filter(
        (pu) => pu.eventTime > this.currentPulseTime
      );
