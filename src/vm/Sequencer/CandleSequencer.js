...
class CandleSequencer {
  ...
  constructor(symbols, timeframes, candleProxy, candleProcessor) {
    ...
    this.pulseTF = timeframes.sort(
      (a, b) => TF.get(a).length - TF.get(b).length
    )[0];
...
*/
  ...
  async processHistoryBuffersPart() {
    ...
    while (pc) {
      ...
      for (var tb of this.tbuffers) {
        ...
        const cdls = tb.fetchCandles(this.lastPulseTime);
        for (let c of cdls) {
  ...
  async loadAndProcessHistory(timeStart, timeEnd) {
    ...
    while (currentEnd < timeEnd) {
      ...
      console.log(
        'CSEQ: load/process history part ' +
          currentPart +
          '/' +
          maxParts +
          ' [' +
      ...
      historyLoadStats
        .filter((s) => !s.res)
        .forEach((s) => (symbolsToRemove[s.symbol] = 1));
  ...
  removeSymbol(symbol) {
    ...
    if (this.modeLive) {
      // unsubscribe from live candle stream
      const brokerCandles = this.candleProxy.getBroker();
