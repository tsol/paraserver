  ...
  start(clientIO) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('get_chart', (arg) => {
        ...
        if (arg.timestamp) {
          param.timestamp = arg.timestamp;
        }

        clientIO
          .getTickerChart(param)
      ...
      socket.on('list_real_orders', (arg) => {
        let data = clientIO.getRealOrdersList(arg);
        socket.emit('real_orders', data);
      });
      ...
      socket.on('get_orders_report', (arg) => {
        try {
          ...
          if (arg.dateFrom) {
            arg.dateFrom = new Date(arg.dateFrom).getTime();
          }
          if (arg.dateTo) {
            arg.dateTo = new Date(arg.dateTo).getTime();
          }
